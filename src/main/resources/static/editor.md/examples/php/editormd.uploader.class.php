    ...
    */
    ...
    class EditorMdUploader
    {
        ...
        public function upload($name)
        {
            ...
            {
                $this->type($this->errors['not_exist']);
            ...
            {
                $this->type($this->errors['unwritable']);
        ...
        private function moveFile()
        {
            ...
            if ($this->formats != "" && !in_array($this->fileExt, $this->formats))
            {
                ...
                $type = "您上传的文件" . $files["name"] . "是" . $this->fileExt . "格式的，系统不允许上传，您只能上传" . $formats . "格式的文件。";
                $this->type($type);
            ...
            {
                $type = "您上传的 " . $files["name"] . "，文件大小超出了系统限定值" . $this->maxSize . " KB，不能上传。";
                $this->type($type);
            ...
            if (!$this->cover) //当不能覆盖时
            {
                ...
                {
                    $this->type($this->saveName . $this->errors['same_file']);
            ...
            if (!@move_uploaded_file($files["tmp_name"], iconv("utf-8", "gbk", $this->savePath . $this->saveName)))
            {
                switch($files["errors"])
                {
                    ...
                    case '1':
                        $type = "上传的文件超过了 php.ini 中 upload_max_filesize 选项限制的值";
                        break;

                    case '2':
                        $type = "上传文件的大小超过了 HTML 表单中 MAX_FILE_SIZE 选项指定的值";
                        break;
        ...
        public function type($type, $success = 0)
        {
            $array = array(
                'success' => $success
            );
            ...
            if ($this->redirect)
            {
                $this->redirectURL .= "&success=" . $success . "&type=" . $type;
