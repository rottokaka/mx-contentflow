...
(function (window, $) {
    ...
    function Wysiwyg(element, userOptions) {
        ...
        var toolbarBtnSelector = "a[data-" + options.commandRole + "],button[data-" + options.commandRole + "],input[contentType=button][data-" + options.commandRole + "]";
            ...
            if ($(gGal).has("img").length) {
                ...
                $.each(gImages, function (i, v) {
                    if ($(v).attr("src").match(/^data:image\/.*$/)) {
                        ...
                        $(gEditor).prepend("<input value='" + $(v).attr("src") + "' contentType='hidden' name='postedimage/" + i + "' />");
    ...
    Wysiwyg.prototype.insertFiles = function (files, options, editor, toolbarBtnSelector) {
        ...
        $.each(files, function (idx, fileInfo) {
            if (/^image\//.test(fileInfo.type)) {
                ...
                options.fileUploadError("unsupported-file-contentType", fileInfo.type);
            }
        });
    };
    ...
    Wysiwyg.prototype.bindToolbar = function (editor, toolbar, options, toolbarBtnSelector) {
        ...
        toolbar.find("input[contentType=text][data-" + options.commandRole + "]").on("webkitspeechchange change", function () {
        ...
        toolbar.find("input[contentType=file][data-" + options.commandRole + "]").change(function () {
            self.restoreSelection();
